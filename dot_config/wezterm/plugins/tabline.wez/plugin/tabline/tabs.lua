...
local function tab_bar_background()
  local theme = config.theme
  if theme.colors and theme.colors.tab_bar and theme.colors.tab_bar.background then
    return theme.colors.tab_bar.background
  end
  if theme.normal_mode and theme.normal_mode.c then
    return theme.normal_mode.c.bg
  end
  return theme.tab.inactive.bg
end
...
local function create_attributes(hover)
  ...
  local bar_bg = tab_bar_background()
  active_attributes = {
  ...
  -- Use tab bar background (not tab bg) so powerline dividers stay visible
  -- between inactive tabs; upstream sets both fg/bg to inactive.bg which hides them.
